...
window.onload =function (){
    ...
    console.log('주문 관리 페이지로 부터 받아온 수정할 데이터 (백엔드로 넘겨줘야할 데이터) : ', typeof requestParameter,requestParameter)
...
const postModifiedData = ()=>{
    var requestOptions = {
      ...
      fetch("http://localhost:3000/products/6544b441b25cf1e3d2e406f8", requestOptions)
        .then(response => response.text())
        .then(result => console.log(result))
        .catch(error => console.log('error', error));
